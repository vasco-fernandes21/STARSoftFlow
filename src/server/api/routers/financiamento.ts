...
const financiamentoBaseSchema = z.object({
  ...
  overhead: z.coerce.number().min(0).max(100).transform(val => new Prisma.Decimal(val)),
  taxa_financiamento: z.coerce.number().min(0).max(100).transform(val => new Prisma.Decimal(val)),
  valor_eti: z.coerce.number().min(0).transform(val => new Prisma.Decimal(val)),
});
...
export const financiamentoRouter = createTRPCRouter({
  ...
  create: protectedProcedure
    .input(createFinanciamentoSchema)
  ...
  update: protectedProcedure
    .input(updateFinanciamentoSchema)
    .mutation(async ({ ctx, input }) => {
      const { id, ...data } = input;
      ...
      try {
        return await ctx.db.financiamento.update({
          ...
          data: {
            ...
            taxa_financiamento: data.taxa_financiamento ? new Prisma.Decimal(data.taxa_financiamento) : undefined,
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        if (projetosComFinanciamento > 0) {
          throw new TRPCError({
            code: "CONFLICT",
            message: "Não é possível eliminar um tipo de financiamento que está a ser usado em projetos",
          });
        }
