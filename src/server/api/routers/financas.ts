...
async function getTotais(db: Prisma.TransactionClient, projetoId: string, options?: { 
  ...
  const alocacoesPassadas = alocacoes.filter(a => 
    a.ano < anoAtual || (a.ano === anoAtual && a.mes < mesAtual) // Mês < mesAtual para incluir o mês anterior mas não o atual
  );
  ...
  const detalhesAnuais = await Promise.all(
    anos.map(async (ano) => {
      ...
      const alocacoesPassadasAno = alocacoes.filter(a => 
        a.ano === ano && (ano < anoAtual || (ano === anoAtual && a.mes < mesAtual)) // Ajuste na lógica do mês
      );
  ...
  getFinancas: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      try {
        ...
        // Usar a função auxiliar para calcular alocações passadas (precisa ser adaptada ou usar a lógica de custosConcluidos)
        // Nota: calcularAlocacoesPassadas parece calcular o custo base (salario * ocupacao), enquanto getTotais usa a fórmula ajustada.
        // Vamos usar os custos já calculados em getTotais para consistência.
        ...
        return {
          ...
          custosRealizados: { // Renomeado de custosConcluidos para mais clareza
            alocacoes: {
            ...
            percentagemOrcamentoReal: orcamentoReal.total.greaterThan(0)
              ? totaisFinanceiros.custosConcluidos.total.dividedBy(orcamentoReal.total).times(100).toNumber()
              : 0,
            percentagemOrcamentoSubmetido: orcamentoSubmetido.orcamentoTotal.greaterThan(0)
              ? totaisFinanceiros.custosConcluidos.total.dividedBy(orcamentoSubmetido.orcamentoTotal).times(100).toNumber()
              : 0
          },
  ...
  getTotaisFinanceiros: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      try {
        ...
        // Função auxiliar para converter detalhes de recursos
        const mapDetalhesRecursos = (detalhes: Awaited<ReturnType<typeof getOrcamentoReal>>['detalhesRecursos']) => 
          detalhes?.map(dr => ({
            userId: dr.userId,
            userName: dr.userName,
            salario: dr.salario?.toNumber() || 0,
        ...
        const dadosFinanceiros = await Promise.all(
          projetos.map(async (projeto) => {
            try {
              ...
              const progressoFisico = totalTarefas > 0 ? Math.round((tarefasConcluidas / totalTarefas) * 100) : 0;

              // Chamar getTotais com incluirDetalhesPorAno: true
              const financasRaw = await getTotais(ctx.db, projeto.id, { incluirDetalhesPorAno: true });
              ...
              const financasMapped = {
                ...
                anos: ('anos' in financasRaw && Array.isArray(financasRaw.anos)) ? financasRaw.anos : [],
                detalhesAnuais: ('detalhesAnuais' in financasRaw && Array.isArray(financasRaw.detalhesAnuais)) 
              ...
              console.error(`Erro ao obter dados financeiros detalhados do projeto ${projeto.id}:`, error);
              return {
                id: projeto.id,
                nome: projeto.nome,
                estado: projeto.estado,
        ...
        const projetosComFinancas = dadosFinanceiros.filter(p => p.financas !== null) as Array<Omit<typeof dadosFinanceiros[number], 'financas'> & { financas: NonNullable<typeof dadosFinanceiros[number]['financas']> }>;

        // Calcular totais consolidados
        const totaisConsolidados = {
          totalProjetosConsultados: projetos.length,
          projetosComFinancas: projetosComFinancas.length,
          orcamentoTotalSubmetido: projetosComFinancas.reduce((acc, p) => acc + p.financas.orcamentoSubmetido, 0),
          valorTotalFinanciado: projetosComFinancas.reduce((acc, p) => acc + p.financas.valorFinanciado, 0),
          custosReaisTotal: projetosComFinancas.reduce((acc, p) => acc + p.financas.custosReais.total, 0),
          custosConcluidosTotal: projetosComFinancas.reduce((acc, p) => acc + p.financas.custosConcluidos.total, 0), 
        ...
        const projetosClassificados = {
          saudaveis: projetosComFinancas.filter(p => p.financas.orcamentoSubmetido > 0 && (p.financas.custosReais.total / p.financas.orcamentoSubmetido) <= 0.7).length,
          emRisco: projetosComFinancas.filter(p => p.financas.orcamentoSubmetido > 0 && (p.financas.custosReais.total / p.financas.orcamentoSubmetido) > 0.7 && (p.financas.custosReais.total / p.financas.orcamentoSubmetido) <= 0.9).length,
          criticos: projetosComFinancas.filter(p => p.financas.orcamentoSubmetido > 0 && (p.financas.custosReais.total / p.financas.orcamentoSubmetido) > 0.9).length,
        ...
        return {
          ...
          taxaMediaFinanciamento: orcamentoTotalValido > 0 
            ? (totaisConsolidados.valorTotalFinanciado / orcamentoTotalValido) * 100 
            : 0,
          margemMedia: orcamentoTotalValido > 0 
            ? (totaisConsolidados.resultadoTotal / orcamentoTotalValido) * 100 
            : 0,
          progressoFinanceiroMedio: totaisConsolidados.custosReaisTotal > 0
            ? (totaisConsolidados.custosConcluidosTotal / totaisConsolidados.custosReaisTotal) * 100
            : 0,
          progressoFisicoMedio: numProjetosValidos > 0 
