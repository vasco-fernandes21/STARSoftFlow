...
export function ProjetoProgressoFinanceiro({ projetos, className }: ProjetoProgressoFinanceiroProps) {
  const router = useRouter();
  
  if (!projetos || projetos.length === 0) {
    return (
      <Card className={cn("glass-card border-white/20 shadow-md transition-all duration-300 ease-in-out hover:shadow-lg", className)}>
        <CardHeader className="border-b border-slate-100/50 px-6 py-4">
          ...
          <CardDescription className="text-slate-500">Progresso financeiro dos projetos ativos</CardDescription>
        </CardHeader>
        <CardContent className="p-6">
          <p className="text-center py-8 text-slate-500">Não há projetos ativos no momento.</p>
        </CardContent>
      </Card>
    );
  }
            ...
            <div key={projeto.id} className="p-4 hover:bg-slate-50/50 transition-colors">
              ...
              <div className="space-y-1 mt-3">
                ...
                <Progress 
                  ...
                  className="h-1.5" 
                  indicatorClassName={cn(
                    projeto.percentualGasto > 100 ? "bg-red-500" :
                    projeto.percentualGasto > 80 ? "bg-amber-500" :
                    "bg-blue-500"
                  )}
                />
              </div>
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                <Badge className="text-xs font-normal px-2 py-0.5 bg-blue-50 text-blue-600 hover:bg-blue-100 border-blue-100">
