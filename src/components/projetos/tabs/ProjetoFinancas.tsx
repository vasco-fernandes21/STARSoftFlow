...
import { Wallet, DollarSign, TrendingUp, ArrowUpDown, Percent, FileText, Calendar } from "lucide-react";
...
function StatCard({ title, value, icon, colorClass = "bg-blue-50", subtitle, tooltipText }: StatCardProps) {
  const cardContent = (
...
export function ProjetoFinancas({ projetoId }: ProjetoFinancasProps) {
  ...
  // --- Helper Functions --- 
  const getOrcamentoStatus = (orcamentoTotal: number | undefined | null, custosReaisTotal: number | undefined | null): OrcamentoStatus => {
    if (typeof orcamentoTotal !== 'number' || isNaN(orcamentoTotal) || orcamentoTotal <= 0 || 
        typeof custosReaisTotal !== 'number' || isNaN(custosReaisTotal)) {
  ...
  if (!isFinancasComDetalhes(financas)) {
     return (
      <div className="rounded-lg bg-gray-50 p-8 text-center">
        <p className="text-sm text-gray-500">Dados financeiros incompletos ou não disponíveis para análise detalhada.</p>
      </div>
    );
  }
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">
              <StatCard 
                ...
                icon={<TrendingUp className={`h-6 w-6 ${(displayData.resultado ?? 0) >= 0 ? 'text-green-600' : 'text-red-600'}`} />}
              ...
              <StatCard 
                ...
                icon={<Percent className={`h-6 w-6 ${(displayData.margem ?? 0) >= 0 ? 'text-green-600' : 'text-red-600'}`} />}
          ...
          <Card className="overflow-hidden border-none shadow-sm">
            <CardHeader className="pb-2">
              <CardTitle className="text-lg font-medium">Progresso Financeiro {selectedYear !== 'todos' ? `(${selectedYear})` : '(Total)'}</CardTitle>
            </CardHeader>
            <CardContent className="pb-6">
              ...
              {dadosGraficoComparacao.length > 0 && (
                <div className="mt-6">
                  <h3 className="text-sm font-medium text-gray-900 mb-4">Comparação Anual (Geral)</h3>
                  <div className="h-80">
          ...
          <Card className="overflow-hidden border-none shadow-sm">
            ...
            <CardContent>
              ...
              {detalhesAnuais.length > 0 ? (
                <>
                  <div className="h-80 mb-6">
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      ...
                      >
                        ...
                        <RechartsTooltip formatter={(value: number) => formatCurrency(value)} labelFormatter={(label: string) => `Ano: ${label}`}/>
                      ...
                      <tbody>
                        ...
                        {detalhesAnuais.map((detalhe: DetalheAnualMapped) => (
                          <tr key={detalhe.ano} className="border-b border-gray-100 hover:bg-gray-50">
                            ...
                            <td className="px-4 py-3 text-right">{formatPercentage(detalhe.margem)}</td>
                            <td className="px-4 py-3 text-right">{formatCurrency(detalhe.vab)}</td>
                            <td className="px-4 py-3 text-right">{formatNumber(detalhe.vabCustosPessoal)}</td>
                          </tr>
                        ))}
                        ...
                        <tr className="bg-gray-50 font-medium">
                          ...
                          <td className="px-4 py-3 text-right">{formatCurrency(financas.valorFinanciado)}</td>
                          <td className="px-4 py-3 text-right">{formatPercentage(financas.margem)}</td>
                          <td className="px-4 py-3 text-right">{formatCurrency(financas.vab)}</td>
                          <td className="px-4 py-3 text-right">{formatNumber(financas.vabCustosPessoal)}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
                  ...
                  <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-3">
                    <div className="rounded-lg bg-blue-50 p-4">
                      ...
                      <p className="text-2xl font-semibold text-blue-900">{formatPercentage(financas.taxaFinanciamento)}</p>
                    </div>
                    <div className="rounded-lg bg-blue-50 p-4">
                      <h3 className="text-sm font-medium text-blue-800">Valor ETI Médio</h3>
                      <p className="text-2xl font-semibold text-blue-900">
                        {formatCurrency(totalEtis > 0 ? (financas.orcamentoSubmetido ?? 0) / totalEtis : 0)}
                      </p>
                    </div>
                    <div className="rounded-lg bg-blue-50 p-4">
                ...
                <div className="rounded-lg bg-gray-50 p-8 text-center">
                  <p className="text-sm text-gray-500">Sem dados anuais de orçamento submetido disponíveis.</p>
                </div>
              )}
            </CardContent>
          </Card>
            ...
            <CardContent>
              ...
              {detalhesAnuais.length > 0 ? (
                <>
                  ...
                  <div className="mt-6">
                    <div className={`rounded-lg p-4 ${(financas.resultado ?? 0) >= 0 ? 'bg-green-50' : 'bg-red-50'}`}>
                      <h3 className={`text-sm font-medium ${(financas.resultado ?? 0) >= 0 ? 'text-green-800' : 'text-red-800'}`}>
                        Resultado Financeiro (Total)
                      </h3>
                      <p className={`text-2xl font-semibold ${(financas.resultado ?? 0) >= 0 ? 'text-green-900' : 'text-red-900'}`}>
                  ...
                  <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-3">
                    <div className="rounded-lg bg-gray-100 p-4">
                      <h3 className="text-sm font-medium text-gray-700">Custos Reais Totais (Geral)</h3>
