...
export default function ExportarProjetoButton({ projetoId }: { projetoId: string }) {
  ...
  const handleExportar = async () => {
    ...
    try {
      ...
      const setCellValue = (sheet: ExcelJS.Worksheet, cellAddress: string, value: any, numFormat?: string, style?: Partial<ExcelJS.Style>) => {
        try {
      ...
      const infoRows = [
        ...
        { label: 'Data de Início', value: projeto.inicio ? new Date(projeto.inicio) : '', format: 'dd/mm/yyyy' },
        { label: 'Data de Fim', value: projeto.fim ? new Date(projeto.fim) : '', format: 'dd/mm/yyyy' },
      ...
      const financeRows = [
        { label: 'Orçamento Submetido', value: Number(financas.orcamento.submetido), format: '#,##0.00 €' },
        { label: 'Valor Financiado', value: Number(financas.financeiro.valorFinanciado), format: '#,##0.00 €' },
        { label: 'Custo Real (Recursos)', value: Number(financas.orcamento.real.recursos), format: '#,##0.00 €' },
        { label: 'Custo Real (Materiais)', value: Number(financas.orcamento.real.materiais), format: '#,##0.00 €' },
      ...
      const dataFim = projeto.fim ? new Date(projeto.fim) : new Date(dataInicio.getTime() + 30 * 24 * 60 * 60 * 1000);
      ...
      if (projeto.workpackages && projeto.workpackages.length > 0) {
        for (const wp of projeto.workpackages) {
          ...
          for (const [userId, recursos] of recursosAgrupados.entries()) {
            ...
            // Duração do recurso
            let minAno = Number.MAX_SAFE_INTEGER, minMes = 13, maxAno = 0, maxMes = 0;
            recursos.forEach(r => {
              if (r.ano < minAno || (r.ano === minAno && r.mes < minMes)) { minAno = r.ano; minMes = r.mes; }
              if (r.ano > maxAno || (r.ano === maxAno && r.mes > maxMes)) { maxAno = r.ano; maxMes = r.mes; }
            });
            
            const duracaoRecurso = minAno !== Number.MAX_SAFE_INTEGER ? (maxAno - minAno) * 12 + (maxMes - minMes) + 1 : 1;
            ...
            mesesProjeto.forEach((periodo, i) => {
              ...
              if (alocacao) {
                setCellValue(rhSheet, `${colLetter}${currentRow}`, Number(alocacao.ocupacao), '0.00%');
              }
            });
        ...
        for (const wp of projeto.workpackages) {
          ...
          if (wp.materiais && wp.materiais.length > 0) {
            for (const material of wp.materiais) {
              ...
              setCellValue(materiaisSheet, `E${currentRow}`, mapeamento[material.rubrica] || "Materiais");
              ...
              setCellValue(materiaisSheet, `H${currentRow}`, preco * material.quantidade, '#,##0.00 €');
      ...
      toast.error(`Ocorreu um erro ao exportar: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
      setIsLoading(false);
    }
  };
