    ...
    <div className="rounded-2xl bg-white p-8">
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <Button
          variant="ghost"
          size="sm"
          className="text-gray-500 hover:bg-azul/5 hover:text-azul"
      ...
      <div className="flex items-center gap-4">
        ...
        />
        <span className="text-sm font-medium text-azul/80">{Math.round(percentualProgresso)}%</span>
      </div>

      {/* Navegação por fases */}
      <div
        className="mt-8 grid"
        style={{ gridTemplateColumns: `repeat(${fasesOrdem.length}, 1fr)` }}
      >
        {fasesOrdem.map((fase) => {
          ...
          return (
            <button
              ...
              className={`group flex flex-col items-center space-y-3 transition-all duration-300 ${isFaseAtual ? "opacity-100" : "opacity-60 hover:opacity-100"} `}
            >
              <div
              ...
              <span
                className={`px-1 text-center text-xs font-medium transition-all duration-300 ${isFaseAtual ? "text-azul" : "text-gray-500 group-hover:text-gray-900"} `}
              >
                {fases[fase].titulo}
              </span>
            </button>
