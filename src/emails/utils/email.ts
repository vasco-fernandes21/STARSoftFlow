...
export async function sendPrimeiroAcessoEmail(email: string, nome: string, token: string): Promise<void> {
  await sendEmail({
    to: email,
    type: "welcome",
    variables: {
...
export async function sendPasswordResetEmail(email: string, nome: string, token: string): Promise<void> {
  await sendEmail({
    to: email,
    type: "resetPassword",
    variables: {
